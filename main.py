...
def post_content_scraper_thread(url, count,content_list):
    ...
    with urlopen(url) as response:
        ...
        # Text
        text = soup.find("div", "discussion-box-content")\
                    .find("p").text.\
                        split("                                        Posted By: ")[0][46:]
            ...
            for post in soup.find_all('div','discussion-box-reply'):
                ...
                for child in children:
                    if "class" in child.attrs and\
                        child.attrs["class"][0] == "discussion-box-reply":
                        break

                    if "class" in child.attrs and\
                        child.attrs["class"][0] == "discussion-box-reply-comment":
...
def child_comment_scraper(parent_num, children_list):
    ...
    Note: this has very simular capabilities, but was
    left seperate due to small changes in comment structure.
    """
    output_list = []
    count = 0
...
# This is taken care of by click
